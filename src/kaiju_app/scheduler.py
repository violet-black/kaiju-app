"""Internal asyncio task scheduler."""
...
from typing import Any, Awaitable, Callable, Mapping, final
from weakref import proxy

from kaiju_app.utils import retry, timeout
...
class ExecPolicy(Enum):
    """Scheduled task execution policies.

    An execution policy determines how and when a scheduled task will be restarted if the previous run has not finished
    yet. The policy is set on creating a task.
    ."""
    ...
    See :py:attr:`~kaiju_app.scheduler.Scheduler.wait_task_timeout_safe_mod`. On scheduler exit this task
...
class ScheduledTask:
    ...
    def __init__(
        ...
        self._executed_task: asyncio.Task | None = None
    ...
    def run(self) -> None:
        self._executed_task = self._scheduler._loop.create_task(self._run(), name=self.name)
    ...
    async def _run(self) -> None:
        loop_time = self._scheduler._loop.time()
...
@dataclass
class Scheduler:
    """Scheduler allows to execute local asynchronous functions periodically."""

    wait_task_timeout_safe_mod: float = 3.0
    """Timeout modifier for WAIT tasks.

    Although a task with :py:attr:`~kaiju_app.scheduler.ExecPolicy.WAIT` policy is supposed to wait for the previous
    run to start the next one, there still should be some limit for this. The scheduler will multiply the `interval_s`
    by this value and set it as the maximum allowed timeout for such task if there's no `max_timeout_s` specified for
    the task.
    """
    ...
    async def stop(self) -> None:
        """Close and cancel all the tasks."""
        self._started = False
        _tasks = []
        for task in self._tasks:
            task.disable()
