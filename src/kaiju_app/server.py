"""Asyncio local server."""

import asyncio
from contextvars import ContextVar
from dataclasses import dataclass, field
from types import MappingProxyType
from typing import Any, Awaitable, Callable, Collection, Iterable, Final, Mapping

from template_dict import Template

from kaiju_app.bases import Logger
from kaiju_app.errors import Timeout, ServiceNotAvailable, Cancelled
...
@dataclass
class Server:
    """Simple asyncio server for internal functions."""

    logger: Logger
    """Optional logger instance."""

    context: ContextVar[_Context] = ContextVar("Server", default=None)
    """Context variable for contextual function call data."""
    ...
    def call_nowait(
        ...
        *,
        ...
        :raises asyncio.QueueFull: if the server is full, use :py:attr:`~kaiju_app.server.Server.full` or
            :py:attr:`~kaiju_app.server.Server.server_not_full` event to check before submitting a request
    ...
    def call_many_nowait(
        ...
        *,
        ...
        :param abort_batch_on_error: abort the whole batch on a first exception, all subsequent requests in the batch
            will return an :py:class:`~kaiju_app.server.Server.Aborted` exceptions
        :param callback: The callback function which will be called with the result
        :param retries: How many times to retry the call
        :param retry_interval_s: How long to wait before retries
        :param task_name: custom asyncio task name
    ...
    def call_template_nowait(
        ...
        *,
        ...
        The difference from :py:meth:`~kaiju_app.server.Server.call_many_nowait` is that this method executes
        ...
        This method is a bit slower than :py:meth:`~kaiju_app.server.Server.call_many_nowait` for obvious
    ...
    async def _call(
        ...
        try:
            ...
            result = Cancelled("Cancelled by the server")
                ...
                for n, (func, args, kws) in enumerate(batch):
                    try:
                        ...
                        if abort_batch_on_error:
                            results.extend([Cancelled("Cancelled by the server")] * (len(batch) - n - 1))
                            break
        except asyncio.TimeoutError:
            results.extend([Timeout("Timeout")] * (len(batch) - n))
        except asyncio.CancelledError:
            results.extend([Cancelled("Cancelled by the server")] * (len(batch) - n))
        finally:
            self._decrement_counter()
            if callback is not None:
