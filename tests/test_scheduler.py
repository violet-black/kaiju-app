...
from kaiju_app.scheduler import Scheduler, ExecPolicy
...
class TestScheduler:
    ...
    async def test_policy_wait(self):
        ...
        async with Scheduler() as sh:
            sh.schedule_task(service.run, interval_s=0.01, policy=ExecPolicy.WAIT)
    ...
    async def test_policy_shield(self):
        ...
        async with Scheduler() as sh:
            sh.schedule_task(service.run, interval_s=0.01, policy=ExecPolicy.SHIELD)
            await asyncio.sleep(0.03)
